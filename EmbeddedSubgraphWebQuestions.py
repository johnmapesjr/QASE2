...
class WebQuestion():
    ...
    def score_path(self, candidate_answers, ques, path):
        ...
        #subgraph = []
        #for a in candidate_answers:
        #    subgraph += [x[0] for x in list(self.c.execute('SELECT DISTINCT C FROM Freebase where A=?', (a,)))]
        #subgraph = list( set( subgraph ) ) #remove redundant data
        #chimeric paper subgraph
        #subgraph_score = self.multiscore(subgraph, ques)
        #average subgraph, does not work as good as paper chimeric.
        #for item in subgraph:
        #    subgraph_score += self.score(ques, item)
        #try:
        #    subgraph_score = subgraph_score / len(subgraph)
        #except:
        #    warning('candidate_answers were:%s No Subgraph...' % (candidate_answers))
        ...
        #return subgraph_score
        #return answer_score + path_score + subgraph_score
        return answer_score + path_score
